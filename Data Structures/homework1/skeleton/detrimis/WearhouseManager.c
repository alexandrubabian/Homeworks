...
Package * create_package(long priority,
  ...
  Package * pachet_covid19;
  pachet_covid19 = malloc(sizeof(Package));
  pachet_covid19 -> priority = priority;
  //pachet_covid19->destination=NULL;
  if (destination != NULL) {
    //pachet_covid19->destination=malloc(sizeof(char)*100);
    pachet_covid19 -> destination = strdup(destination);
  } else
    pachet_covid19 -> destination = NULL;
  return pachet_covid19;
}
